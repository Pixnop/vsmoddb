...
switch($urlparts[0]) {
	...
	case 'dependency-graph':
		...
		// Cycle detection: any back edge in the required-only DFS gets the inCycle flag so the
		// client can highlight it. See findCycleEdgeIndices in lib/relations.php.
		foreach (findCycleEdgeIndices($edges) as $idx) {
			$edges[$idx]['inCycle'] = true;
		}
